...
let sum_board =
  List.fold_left (List.fold_left (fun a x -> if x == -1 then a else a + x)) 0
...
let rec run_game calls boards =
  ...
  | call :: tail ->
      ...
      let lose_boards = List.filter (fun b -> not (is_winner b)) new_boards in
      if List.length lose_boards > 0 then
        run_game tail lose_boards
      else
