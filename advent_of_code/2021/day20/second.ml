...
let rec apply_n_times algo image default n =
  if n <= 0 then image else
  apply_n_times algo (apply_algo algo default image) (not default) (n - 1)

let () =
  let algo, image_grid = read_input () in
  let image = grid_to_set image_grid in
  let final_image = apply_n_times algo image false 50 in
  Printf.printf "Ans: %d\n" (count_on final_image)
